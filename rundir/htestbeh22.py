...
def main_opt(*args , **kwargs):
    ...
    for basis in basissets:
        hamfiles[basis] = fc.File_Collector(elemdir , search = search %basis ,notsearch = r'(\.sw\w)|(unitary)',sortfunction = fileinfo, filterf =  lambda x : fileinfo(x) >= 5.1 and fileinfo(x) <= 1000. )
    ...
    for basis in basissets:
        ...
        dw.cimain(psir.values['nalpha']-1,psir.values['nbeta']-1 ,psir.values['norb']-1,[[] , []], [0,4] ,fname = detfile30 ,ref =  [lambda x , y , z : psir.get_hf_orbs()] , add_frozen = 1) #CISD
