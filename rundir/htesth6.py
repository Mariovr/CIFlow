...
def main_opt(*args , **kwargs):
    ...
    for basis in basissets:
        hamfiles[basis] = fc.File_Collector(elemdir , search = search %basis ,notsearch = r'(\.sw\w)|(unitary)',sortfunction = fileinfo, filterf =  lambda x : fileinfo(x) >= 0.3 and fileinfo(x) <= 1000. )
