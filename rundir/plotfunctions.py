...
class Plot_Files(object):
    def __init__(self, name , searchstr = 'plotenergy' , notsearch =r'\.swp|\.png' , regexp = None, sortfunction = None):
        ...
        self.fig = pl.figure(1,figsize=(8,6), dpi=100 , frameon = True , facecolor = '0.75' , edgecolor = 'w')
    ...
    def generate_plot(self, xlimg = None , ylimg =None , exname = '' , prefix = True, titel = None ,name = 'energyplot', depcol = 0, ylist = None , save = True, color = ['','','','','','','','','', '','', '' , '' , '' , ''], exdir = './', samedir = False, datanum = 0):
      """
      ...
      for i in ylist:
          self.plotwrap(depcol, i, 'energy' , name =  exname, xlim = xlimg , ylim = ylimg , prefix = prefix ,save = False, label = self.data[datanum].columnheader[i], color = color[i-1], datanum = datanum)
    ...
    def plotwrap(self, xindex, yindex, yas, name = None, titel = None ,color = 'r' , sort = '' , label = None , xlim = None , ylim = None , prefix = False,save = True, datanum = 0):
      self.fig.axes[0].plot(self.data[datanum].data[:len(self.data[datanum].data[:,yindex]),xindex],self.data[datanum].data[:,yindex], color+sort , label = label)
    ...
    def layout(self ,  xlab , ylab , xlim = None , ylim = None , tit = None , axnum = 0 , legendhand = None , legendlab = None , legendpos = 0 , finetuning = False , axbg = None , fs = 22, ticksize = 14):
      """
...
def main():
    ...
    plotter.generate_plot(xlimg = xlim, ylimg = ylim, ylist = [1], titel = title, name = 'plot' , exname = 'short')
    #plotter.generate_plot(xlimg = None, ylimg = None, ylist = [2,4,6], titel = title, name = 'overlapplot' ,  exname = 'sym')
    option = None
