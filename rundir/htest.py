    ...
    for basis in basissets:
        for r in runlist:
            ...
            if package == 'psi':    
                ...
                assert(units == 'au') #horton uses atomic units internally 
                if (os.getenv('VSC_INSTITUTE_LOCAL') != 'gent'):
                    chemham = hf.Chem_Ham(filename = "horton%.2f" %(r) +basis+  ".dat",atoms = atoms, positions = positions,  basis =basis) 
                    chemham.write_to_file()
        filenum += 1        
